...
class CreateColaboradorsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('colaboradors', function (Blueprint $table) {
            ...
            $table->string('nombre');//NOMBRE	Character	50
            $table->string('nombres');//NOMBRES	Character	100
            $table->date('fecnac');//FEC_NAC	Date	8
            $table->string('sexo_id',1)->default('M');
            $table->string('tipdoc_id',2)->default('01');//TIP_DOC	Character	2
            $table->string('numdoc');//NUM_DOC	Character	10
            $table->string('domicilio')->nullable();//DOMICILIO	Character	100
            $table->string('distrito',30)->nullable();//DISTRITO	Character	20
            $table->string('provincia',30)->nullable();//PROVINCIA	Character	20
            $table->string('departamento',30)->nullable();//DEPARTAMENTO	Character	20
            $table->integer('residencia')->default(1);//RESIDENCIA	Numeric	1
            $table->string('lugnac',50)->nullable();//LUG_NAC	Character	100
            $table->string('tieresi',30)->nullable();//TIE_RESI	Character	10
            ...
            $table->text('foto')->nullable();//FOTO	Character	200
            $table->softDeletes();
            $table->timestamps();
        });
    }
